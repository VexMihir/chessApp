export function PrevMoveListPannel({ prop }) {
  ...
  return (
    <div className={"PrevMoveList"} id={"PrevMoveList"} style={style}>
      {childArr.map((child) => {
        return (
          <div key={childArr.indexOf(child)} style={childStyle}>
            <p key={childArr.indexOf(child) + "P1"}>
              {childArr.indexOf(child) + 1}. {child.P1}
            </p>
