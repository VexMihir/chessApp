...
export default function PreviousGameView() {
  ...
  const currPGNObj = JSON.parse(
    useSelector((state) => state.PrevGameView.PGNOBJ)
  );
  ...
  return (
    <>
      <div id={"PrevGameView"} className={"h-fit m-[1rem]"}>
        <div className={"h-[0.5%] items-center m-0"} id={"NavBar"}>
          <NavBar />
        </div>
        <div
          id={"MainComponents"}
          className={
            "min-h-[650px] flex flex-row gap-x-[2rem] items-stretch justify-center p-[2rem] "
          }
        >
          <Chessboard
            width={650}
          ...
          <div
            className={
              "flex flex-col h-[650px] w-[100%] bg-custom-grey rounded-lg justify-evenly " +
              "shadow shadow-[#475569] shadow-lg"
            }
          >
            <div
              className={
                "flex flex-fol h-[50px] w-[95%] p-[1rem] justify-center "
              }
            >
              <NavLink
                className={
                  location.pathname === normalView
                    ? "text-white no-underline font-bold text-lg p-[2rem] mr-2 " +
              ...
              <NavLink
                className={
                  location.pathname === analysisView
                    ? "text-white no-underline font-bold text-lg p-[2rem] mr-2 " +
                      "flex justify-center items-center rounded-xl " +
                      "hover:bg-custom-black font-bold " +
                      "z-1 " +
