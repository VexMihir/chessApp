...
export default function ChessboardGame({socket, setSocket, roomId, isGameStarted, setIsGameStarted, isModalOpen, setIsModalOpen, setResult, activePlayer, setActivePlayer}) {
    ...
    const [legalPieceMovesNotation, setLegalPieceMovesNotation] = useState([]);
    ...
    function onSquareClick(square) {
      ...
      for (let i = 0; i < legalPieceMovesNotation.length; i++) {
        
        validMoves.push(legalPieceMovesNotation[i]['to'])
      }
      ...
      console.log("line 114", validMoves);
    ...
    function onDrop({sourceSquare, targetSquare}) {
      if (socket) {
        let result = "";
        for (let i = 0; i < legalPieceMovesNotation.length; i++) {
          if (legalPieceMovesNotation[i]['to'] === targetSquare) {
            result = legalPieceMovesNotation[i]['san']
            break;
          }
        }
    ...
    useEffect(() => {
      ...
      newSocket.on('moveMade', (move, fen, legalMoves) => {
        ...
        console.log("line 160", legalMoves);
      ...
      newSocket.on('checkmate', (isCheckmate) => {
